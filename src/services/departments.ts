...
export const deleteMultipleDepartments = async (ids: number[]) => {
    const promises = ids.map((id) => deleteDepartment(id));
    return Promise.all(promises); // ejecuta todas las eliminaciones en paralelo
};
